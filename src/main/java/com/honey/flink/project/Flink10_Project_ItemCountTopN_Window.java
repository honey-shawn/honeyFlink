 ...
 *
 */
public class Flink10_Project_ItemCountTopN_Window {
    public static void main(String[] args) throws Exception {
        // 1.获取执行环境
        StreamExecutionEnvironment env = StreamExecutionEnvironment.getExecutionEnvironment();
        env.setParallelism(1);// 单并行度速度慢，可以看的清楚，生产环境可以多并行度

        // 2.读取文本数据
        DataStreamSource<String> readTextFile = env.readTextFile("input/UserBehavior.txt");

        // 3.转换成javBean，根据行为过滤数据，并提取时间戳生产WaterMark
        ...
        SingleOutputStreamOperator<ItemCount> aggregate = keyedStream.window(SlidingEventTimeWindows.of(Time.hours(1), Time.minutes(5)))
                .aggregate(new ItemCountAggFunc(), new ItemCountWidowFunc());
    ...
    public static class ItemCountWidowFunc implements WindowFunction<Integer, ItemCount ,Long ,TimeWindow>{
        @Override
    ...
    public static class ItemCountProcessFunc extends KeyedProcessFunction<String, ItemCount, String>{
        ...
        public void onTimer(long timestamp, OnTimerContext ctx, Collector<String> out) throws Exception {
            ...
            for (int i = 0; i < Math.min(topSize, itemCounts.size()); i++){
                ...
                stringBuilder.append("ItemId:").append(itemCount.getItem());
